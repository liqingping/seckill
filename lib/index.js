...
module.exports = {
	...
	addForTime: function (id, number, option, callback){
		...
		let salt = Math.random().toString().substring(3,8);
		let val = {
		...
		client.setex(`${id}option`, parseInt(time/1000), JSON.stringify(val), function (err, result) {
			...
			client.hincrby(`${id}hash${salt}`, 'userId', 1, function (err) {
				if (err) {
					client.end(true);
					return callback(err, null);
				}
		...
		function seckill(client) {
			...
			client.get(`${id}option`, function (err, result) {
				...
				client.get(id, function (err, reply) {
					...
					client.hget(hashKey, userId, function (err, result) {
						//判断是否达到领取现在
						if (result >= limit) {
							client.end(true);
						...
						//添加领取记录到hash中
						multi.hincrby(hashKey, userId, 1);
						//提交事务
	...
	getForTime: function (id, userId, num, callback){
		let arg = arguments;
		let number = callback ? num : 1;
		...
		function seckill(client) {
			...
			client.get(`${id}option`, function (err, result) {
				...
				client.get(id, function (err, reply) {
					...
					if (~~reply === -1) {
						//获取用户领取详情
						return client.hget(hashKey, userId, function (err, result) {
							//判断是否达到领取限制
							...
							let multi = client.multi();
							//添加领取记录到hash中
							multi.hincrby(hashKey, userId, number);
							//提交事务
							multi.exec(function (err, replies) {
								//事务是否还存在
								if (!replies) {
